  ...
  <body>
    <div class="demo-layout mdl-layout mdl-js-layout mdl-layout--fixed-drawer mdl-layout--fixed-header">
      ...
      <main class="mdl-layout__content mdl-color--grey-100">
        <div class="content">
          <?php
          $req = $bdd->prepare('SELECT * FROM ordonance WHERE idOrdonance = :ordonance');
          $req->execute(array(
            "ordonance" => $_GET["id"]
          ));
          $ordonance = $req->fetch();
          ...
          $req->execute(array(
            "idUser" => $ordonance["idUser"]
          ));
          $user = $req->fetch();
          $req = $bdd->prepare("SELECT * FROM user WHERE idUser = :idMedecin");
          $req->execute(array(
            "idMedecin" => $ordonance["idMedecin"]
          ));
          $medecin = $req->fetch();
          ?>
          <h3>Ordonnance de <?php echo $user["firstName"] . " " . $user["lastName"] ?></h3>
          <p>
            Par <?php echo $medecin["firstName"] . " " . $medecin["lastName"] . " le " . $ordonance['dateCrea']; ?>
          </p>
          <table class="list-medic" id="listMedic">
            ...
            <?php
            $req = $bdd->prepare('SELECT medicament.*, itemOrdonance.* FROM medicament, itemOrdonance WHERE itemOrdonance.idOrdonance = :idOrdonance AND medicament.idMedicament = itemOrdonance.idMedicament');
            $req->execute(array(
              "idOrdonance" =>  $_GET["id"]
            ));
            while ($donnees = $req->fetch())
            {
              ...
              echo "<td>" . $donnees["frequency"] . " " . $data . "</td>";
              echo "<td>" . $donnees["dose"] ."</td>";
              echo "</tr>";
            }
            ?>
          </table>
          <?php if ($_SESSION['typeUser'] == 3) { ?>
            <a href="valid-ordo-<?php echo $_GET['id'];?>"><button>Valider ordonnance</button></a>
            <a href="new-message-<?php echo $ordonance['idMedecin']; ?>"><button>Contacter médecin</button></a>
          <?php }
          if ($_SESSION['typeUser'] == 2) {?>
            <a href="new-message-<?php echo $ordonance['idUser']; ?>"><button>Contacter patient</button></a>
          <?php } ?>
        </div>
      </main>
